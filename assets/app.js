...
// The seven-day training is rendered by PHP from the editorial document.
...
fetch('media.php').then((response) => response.ok ? response.json() : null).then((media) => {
  ...
  const heroCurrent = document.querySelector('.hero-image img');
  if (mainBackgrounds[0] && heroCurrent && !heroCurrent.src.endsWith(mainBackgrounds[0])) heroCurrent.setAttribute('src', mainBackgrounds[0]);
